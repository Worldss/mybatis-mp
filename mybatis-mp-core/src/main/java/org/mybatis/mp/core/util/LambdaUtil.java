...
public final class LambdaUtil {
    ...
    public static <T> String getName(Getter<T> getter) {
        return MapUtil.computeIfAbsent(LAMBDA_GETTER_FIELD_MAP, getter, (key) -> {
    ...
    public static <T> Class<?> getClass(Getter<T> getter) {
        return MapUtil.computeIfAbsent(LAMBDA_GETTER_CLASS_MAP, getter, (key) -> {
    ...
    private static SerializedLambda getSerializedLambda(Getter getter) {
        return MapUtil.computeIfAbsent(SERIALIZED_LAMBDA_MAP, getter, key -> {
            try {
